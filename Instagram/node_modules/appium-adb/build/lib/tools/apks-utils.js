...
async function extractFromApks(apks, dstPath) {
    ...
    return await APKS_CACHE_GUARD.acquire(apks, async () => {
        ...
        if (APKS_CACHE.has(apksHash)) {
            const resultPath = path_1.default.resolve(APKS_CACHE.get(apksHash), ...dstPath);
        ...
        const resultPath = path_1.default.resolve(tmpRoot, ...dstPath);
        if (!await support_1.fs.exists(resultPath)) {
            throw new Error(`${dstPath.join(path_1.default.sep)} cannot be found in '${apks}' bundle. ` +
