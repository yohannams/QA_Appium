...
const getJavaForOs = lodash_1.default.memoize(async function getJavaForOs() {
    ...
    throw new Error(`The '${executableName}' binary could not be found ` +
        `neither in PATH nor under JAVA_HOME (${errMsg || path_1.default.resolve(javaHome, 'bin')})`);
});
...
async function dirExists(location) {
    return location
        && await support_1.fs.exists(location)
        && (await support_1.fs.stat(location)).isDirectory();
}
